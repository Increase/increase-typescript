...
import { APIPromise } from 'increase/core/api-promise';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['INCREASE_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: Increase) => {
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['INCREASE_LOG'] = 'debug';
    const client = new Increase({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['INCREASE_LOG'] = 'not a log level';
    const client = new Increase({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'INCREASE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    test('in request options overridden by client options', () => {
      const client = new Increase({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual('http://localhost:5000/client/foo');
    });

    test('in request options overridden by env variable', () => {
      process.env['INCREASE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Increase({ apiKey: 'My API Key' });
...
describe('idempotency', () => {
  test('key can be set per-request', async () => {
    const client = new Increase({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key' });
