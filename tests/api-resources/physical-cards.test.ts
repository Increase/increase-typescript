...
describe('resource physicalCards', () => {
  ...
  test('update: only required params', async () => {
    const responsePromise = client.physicalCards.update('physical_card_ode8duyq5v2ynhjoharl', { status: 'disabled' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('update: required and optional params', async () => {
    const response = await client.physicalCards.update('physical_card_ode8duyq5v2ynhjoharl', { status: 'disabled' });
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.physicalCards.list({
    card_id: 'card_id',
    created_at: {
