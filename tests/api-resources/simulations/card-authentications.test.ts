...
describe('resource cardAuthentications', () => {
  ...
  test('challengeAttempts: only required params', async () => {
    const responsePromise = client.simulations.cardAuthentications.challengeAttempts(
  ...
  test('challengeAttempts: required and optional params', async () => {
    await client.simulations.cardAuthentications.challengeAttempts('card_payment_nd3k2kacrqjli8482ave', {
      one_time_code: '123456',
    });
  });

  test('challenges', async () => {
    const responsePromise = client.simulations.cardAuthentications.challenges(
      'card_payment_nd3k2kacrqjli8482ave',
    );
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
