...
describe('resource cardDisputes', () => {
  test('create: only required params', async () => {
    const responsePromise = client.cardDisputes.create({ disputed_transaction_id: 'transaction_uyrp7fld2ium70oa7oi', network: 'visa' });
  ...
  test('create: required and optional params', async () => {
    await client.cardDisputes.create({
    ...
    consumer_credit_not_processed: { canceled_or_returned_at: '2019-12-27', credit_expected_at: '2019-12-27' },
    ...
    consumer_original_credit_transaction_not_accepted: { explanation: 'x', reason: 'prohibited_by_local_laws_or_regulation' },
    ...
    paid_by_other_means: { other_form_of_payment_evidence: 'canceled_check', other_transaction_id: 'x' },
  },
  },
  });
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.cardDisputes.list({
    created_at: {
  ...
  test('submitUserSubmission: only required params', async () => {
    const responsePromise = client.cardDisputes.submitUserSubmission('card_dispute_h9sc95nbl1cgltpp7men', { network: 'visa' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('submitUserSubmission: required and optional params', async () => {
    await client.cardDisputes.submitUserSubmission('card_dispute_h9sc95nbl1cgltpp7men', {
    network: 'visa',
    amount: 1,
    attachment_files: [{ file_id: 'file_id' }],
    explanation: 'x',
    ...
    merchant_prearbitration_decline: { reason: 'The pre-arbitration received from the merchantdoes not explain how they obtained permission to charge the card.' },
    user_prearbitration: {
    reason: 'x',
